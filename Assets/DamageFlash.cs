...
public class DamageFlash : MonoBehaviour
{
    ...
    public void TriggerMaterialChange(Color color)
    {
        ...
        changeCoroutine = StartCoroutine(ChangeMaterialsTemporarily(color));
    }


    private IEnumerator ChangeMaterialsTemporarily(Color color)
    {
        ChangeMaterialsToColor(color); // 255, 255, 255
    ...
    private void ChangeMaterialsToColor(Color color)
    {
        ...
        foreach (MeshRenderer renderer in meshRenderers)
        {
            ...
            for (int i = 0; i < renderer.materials.Length; i++)
            {
                ...
                print("COLOR" + newMaterials[i].HasProperty("_BaseColor"));
                if (newMaterials[i].HasProperty("_BaseColor"))
                {
                    newMaterials[i].SetColor("_BaseColor", color);
                } else {
                    newMaterials[i].SetColor("_Color", color);
                }
            }
